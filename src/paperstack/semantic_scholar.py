"""Bounded Semantic Scholar search, metadata, authors, citations, and graph traversal."""
...
PAPER_FIELDS = (
    SEARCH_FIELDS
    + ",publicationDate,publicationTypes,fieldsOfStudy,s2FieldsOfStudy,journal,openAccessPdf,citationStyles"
)
...
def paper(paper_id: str, *, selected_fields: str | None = None) -> dict:
    url = f"{BASE_URL}/paper/{identifier(paper_id)}"
    params = {"fields": fields(selected_fields, PAPER_FIELDS)}
    return _get(url, params)
...
def citation(paper_id: str, *, format: str = "bibtex") -> dict:
    wanted = format.lower()
    if wanted not in FORMATS:
        raise ValueError(f"format must be one of: {', '.join(FORMATS)}")
    result = paper(paper_id, selected_fields="paperId,title,citationStyles")
    if result["status"] != "ok":
        return result
    styles = result.get("response", {}).get("citationStyles") or {}
    if wanted not in styles:
        return metadata._result(
            "semantic_scholar",
            result["url"],
            error=f"{wanted} citation is unavailable; available: {', '.join(sorted(styles)) or 'none'}",
        )
    result["format"] = wanted
    result["citation"] = styles[wanted]
    return result
